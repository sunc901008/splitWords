...
public class MaxMinInstruction {
    ...
    public static JSONArray build(FocusPhrase focusPhrase, int index, JSONObject amb, List<Formula> formulas) throws FocusInstructionException, IllegalException {
        ...
        FocusNode keywordNode = focusNodes.get(0);
        FocusPhrase numberPhrase = focusNodes.get(1).getChildren();

        AnnotationToken token1 = new AnnotationToken();
        token1.addToken(keywordNode.getValue());
        token1.value = keywordNode.getValue();
        token1.type = Constant.AnnotationTokenType.SYMBOL;
        token1.begin = keywordNode.getBegin();
        token1.end = keywordNode.getEnd();
        datas.addToken(token1);

        json1.put("aggregation", keywordNode.getValue().toUpperCase());
