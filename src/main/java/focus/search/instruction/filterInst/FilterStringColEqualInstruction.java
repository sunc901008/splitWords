...
public class FilterStringColEqualInstruction {
    ...
    public static JSONArray build(FocusPhrase focusPhrase, int index, JSONObject amb, List<Formula> formulas) {
        if (focusPhrase.getFocusNodes().get(2).getChildren().size() > 3) {// 多个列中值
            return buildMulti(focusPhrase, index, amb, formulas);
        }
        return buildSingle(focusPhrase, index, amb, formulas);// 单个列中值
    }

    private static JSONArray buildSingle(FocusPhrase focusPhrase, int index, JSONObject amb, List<Formula> formulas) {
    ...
    private static JSONArray buildMulti(FocusPhrase focusPhrase, int index, JSONObject amb, List<Formula> formulas) {
        List<FocusNode> focusNodes = focusPhrase.getFocusNodes();
        FocusNode param1 = focusNodes.get(0);
        FocusNode symbol = focusNodes.get(1);
        symbol = symbol.isHasChild() ? symbol.getChildren().getFirstNode() : symbol;
