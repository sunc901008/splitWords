...
//    private final static int MAX_RULE_LOOP = 50;
//
//    public FocusParser() {
...
//                    List<FocusNode> nodes = new ArrayList<>();
//                    replaceNode(nodes, rules, fn.getValue(), focusToken, 0);
//                    for (FocusNode focusNodeNew : nodes) {
//                        focusPhraseNew.replaceNode(position, focusNodeNew);
//                        if (focusPhraseNew.size() == position + 1 && focusPhraseNew.getNodeNew(position).getValue().equalsIgnoreCase(focusToken.getWord())) {
...
//    }
//
//    private void replaceNode(List<FocusNode> nodes, List<BnfRule> rules, String ruleName, FocusToken focusToken, int max_rule) {
//        if (max_rule >= MAX_RULE_LOOP) {
//            logger.warn("Maximum number of recursion steps reached. If the input string is indeed valid, try increasing the limit.");
//            return;
//        }
//
//        BnfRule br;
//        try {
//            br = findRule(rules, ruleName);
//        } catch (FocusParserException e) {
//            logger.warn(Common.printStacktrace(e));
//            return;
//        }
...
//                if (!first.match(focusToken.getWord())) {
//                    continue;
//                }
//                FocusNode node = new FocusNode(ruleName);
//                FocusPhrase newFp = new FocusPhrase(brName);
//                FocusNode newFirst = new FocusNode(first.getName());
//                newFirst.setValue(first.getName());
//                newFirst.setType(((TerminalToken) first).getType());
//                newFirst.setBegin(focusToken.getStart());
//                newFirst.setEnd(focusToken.getEnd());
//                if (newFirst.getType().equals(Constant.FNDType.INTEGER)
//                        || newFirst.getType().equals(Constant.FNDType.DOUBLE)
//                        || newFirst.getType().equals(Constant.FNDType.COLUMN_VALUE)) {
//                    newFirst.setValue(focusToken.getWord());
//                }
//                newFirst.setColumn(((TerminalToken) first).getColumn());
//                newFirst.setTerminal();
//                newFp.addPn(newFirst);
//                for (int i = 1; i < ts.size(); i++) {
//                    Token next = ts.get(i);
//                    FocusNode newFn = new FocusNode(next.getName());
//                    newFp.addPn(newFn);
//                }
//                node.setChildren(newFp);
//                nodes.add(node);
//            } else {
//                List<FocusNode> tmp = getChildNode(rules, first.getName(), focusToken, max_rule + 1);
//                for (FocusNode t : tmp) {
//                    FocusNode node = new FocusNode(ruleName);
//                    FocusPhrase newFp = new FocusPhrase(brName);
//                    newFp.addPn(t);
...
//    }
//
//    private List<FocusNode> getChildNode(List<BnfRule> rules, String ruleName, FocusToken focusToken, int max_rule) {
//        List<FocusNode> nodes = new ArrayList<>();
//        BnfRule br;
//        try {
//            br = findRule(rules, ruleName);
...
//                    continue;
//                }
//                newFirst = new FocusNode(first.getName());
//                newFirst.setValue(first.getName());
//                newFirst.setType(((TerminalToken) first).getType());
//                newFirst.setBegin(focusToken.getStart());
//                newFirst.setEnd(focusToken.getEnd());
...
//            nodes.add(node);
//        }
//        getChildNode(nodes, rules, focusToken, max_rule + 1);
//        return nodes;
//    }
//
//    private void getChildNode(List<FocusNode> nodes, List<BnfRule> rules, FocusToken focusToken, int max_rule) {
//        int loop = nodes.size();
//        while (loop > 0 && !nodes.isEmpty()) {
//            FocusNode fn = nodes.remove(0);
//            if (fn.isTerminal()) {
//                nodes.add(fn);
//            } else {
//                fn.getChildren().getFocusNodes().
//            }
//        }
//    }
//
